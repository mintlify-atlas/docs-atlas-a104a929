...
<ResponseField name="grade_report" type="GradeReport" required>
  ...
  <Expandable title="GradeReport fields">
    ...
    <ResponseField name="courses" type="CourseGrade[]">
      Array of individual course grade records. See **CourseGrade** below.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<Expandable title="CourseGrade fields">
...
Attached to a `CourseGrade` when the backend can project what minimum scores are needed in pending units for the student to pass.
...
```typescript GradeReport & CourseGrade
...
```python GradeReport (domain entity)
...
```python AcademicRecord (domain entity)
from dataclasses import dataclass, field
from decimal import Decimal
from typing import List, Optional
