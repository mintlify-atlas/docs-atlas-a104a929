---
title: "Unitru Academic System Architecture Overview"
...
<CardGroup cols={2}>
  ...
  <Card title="Browser session destroyed on close" icon="trash">
    `SessionManager` ties each `BrowserSession` to a WebSocket connection. When the socket closes — whether normally or due to an error — the `finally` block calls `destroy_session`, which closes the Playwright resources in strict order: **Page → BrowserContext → Browser**.
  </Card>
