...
description: "The Next.js 16 frontend uses feature-based folder organization. Each feature owns its components and types. Communication with the backend is via a single WebSocket client."
---
...
export function connectAndFetch(
  ...
  ws.onmessage = (event) => {
    ...
    if (message.event === "dashboard_ready") {
      ...
      callbacks.onError((message.data as { message?: string }).message ?? "Error desconocido")
    } else {
...
| **Notas y asistencia** | `extracting_attendance`, `attendance_extraction_success/failed`, `extracting_grades`, `grade_extraction_success`, `optimizing_schedule`, `schedule_optimized/failed` |
...
- **Left**: `SidebarNav` — a vertical list of tab links. On mobile it is an off-canvas drawer toggled by a hamburger button. Includes the student's name, an attention badge when any course is at attendance risk, and a "Cerrar sesión" / reset link.
